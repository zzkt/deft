...
<<<<<<< HEAD
The =define= macro replaces the need to choose between =defn=, =deftfn=, =deftn=, =def=, and =deftval= and provides a smooth (gradual) way of moving from untyped to typed code and back.
=======
...
<<<<<<< HEAD
Define a gradually typed function. Every parameter must be followed by a type spec, the return type comes after the parameter vector. =deftfn= inserts runtime arg and return checks but does *not* register an =:fn= type scheme, so =fn-type-of= will not work for functions defined with =deftfn= (use =deftn= or =define= if you need to query a function's type at runtime).

A trailing =&= ignores extra arguments (see Janet's "Ignoring extra arguments"), and a trailing =& rest-name= collects the rest into an untyped tuple.
=======
Define a typed function. Every parameter *must* be followed by a type spec, the return type comes after the parameter vector. ~deftfn~ inserts runtime arg and return checks but does *not* register a ~:fn~ type scheme (so ~fn-type-of~ will not work for functions defined with ~deftfn~, use ~define~ if you need to query a function's type at runtime).
>>>>>>> t0
...
(deftfn ignore-extra [x :number &] :number
  (+ x 1))
(ignore-extra 1 2 3 4 5)  # => 2

# Variadic — the rest tuple is untyped
(deftfn total [x :number & xs] :number
  (+ x (length xs)))
(total 1 2 3 4)  # => 4
#+end_src
...
Like =deftfn= but defines a private function (c.f =defn-=)

<<<<<<< HEAD
*** =deftn=

Define a function with optional type annotations. Parameters without an explicit type are inferred from usage in the function body (when inference is enabled) or default to =:dynamic=. The return type is optional and defaults to =:dynamic= when omitted.
...
...and without annotations =deftn= behaves the same as a plain =defn=
=======
...
Optional arguments are declared after ~&opt~ and default to ~nil~
>>>>>>> t0
#+BEGIN_SRC janet
(define options [a b c &opt d e f] :number
  (default d 10)
  (default e 11)
  (default f 12)
  (+ a b c d e f))
...
#+BEGIN_SRC janet
(define long [x :number & xs] :number
  (+ x (length xs)))
...
Optional arguments with =&opt= default to =nil= and work as expected with the =default= macro
#+BEGIN_SRC janet
(deftn opt [a b c &opt d e f]
  (default d 10)
  (default e 11)
  (default f 12)
  (+ a b c d e f))
...
Variadic functions gather 'rest' into a tuple, adding a type after the rest name types each element
#+BEGIN_SRC janet
(deftn sum [& xs :number]
  (var total 0)
  (each n xs (+= total n))
...
(deftn mix [x :number &opt y :number & tail :number]
  (default y 0)
  (+ x y (length tail)))
#+END_SRC

Ignoring extra arguments (with a bare =&=)
#+BEGIN_SRC janet
(deftn ignore-extra [x :number &]
  (+ x 1))
(ignore-extra 1 2 3 4 5)  # => 2
#+END_SRC

Keyword arguments with =&keys= bind named keyword parameters to local symbols
#+BEGIN_SRC janet
(deftn greet [name :string &keys {:greeting :string}]
  (string greeting ", " name "!"))
(greet "World" :greeting "Hello")  # => "Hello, World!"

(deftn draw [x :number &opt y :number &keys {:color :string}]
  (default y 0)
  (string color ": " (+ x y)))
...
<<<<<<< HEAD
Define a named structural record type with typed field declarations. A definition will also generate a constructor, accessors, mutators (if mutable), and an optional =:pp= pretty-print handler.
=======
...
*Field clauses*
<<<<<<< HEAD
- =(field name type)= — required positional argument
- =(optional name type)= — optional positional argument (defaults to =nil=), can also be set via =:keyword value=
- =(guard pred-fn)= — optional guard predicate (for any extra validation)
- =(print handler-fn)= — optional custom pretty-print handler (for string based output)
=======
- ~(field name type)~ — required positional argument
- ~(optional name type)~ — optional positional argument (defaults to ~nil~), can also be set via ~:keyword value~
- ~(guard pred-fn)~ — optional guard predicate (for any extra validation)
...
#+BEGIN_SRC janet
<<<<<<< HEAD
(def p1 (make-person "Alice" 30)) # only required fields
(def p2 (make-person "Robert');" 13 "Mr." "Little Bobby Tables")) # optional positional
=======
...
A definition can include guard predicates which have access to declared fields. A guard predicate follows the same semantics as a type predicate. A guard can extend the validation of a single field (c.f. type narrowing) and also validate relations between fields. e.g. a guard can be used to check that an =end= field of type =:datetime= occurs later than the =start= field.
...
Define an enumeration type using a string to value map. Values can be of any =:type=. =defenum= registers a type predicate and generates an accessor for looking up values by key. The functions '<name>-extend' and '<name>-remove' are also generated and can be used to modify the enum.
...
<<<<<<< HEAD
The =-extend= function adds or updates a key-value pair in the enum table. The key must be a =:string= and the value must match the declared =:type=
=======
...
<<<<<<< HEAD
The =-remove= function removes a key from the enum table and returns its previous value (or =nil= if not found).
=======
...
<<<<<<< HEAD
Check if a value's type matches a given type. Uses the extended =type= that accounts for user-declared types.
=======
...
<<<<<<< HEAD
The consistency relation from Siek & Taha 2006. =:dynamic= is consistent with every type, otherwise requires equality. Used internally as the core relation in gradual type checking.
=======
...
=======
Type variables are resolved through unification with gradual semantics (i.e. =:dynamic= unifies with anything, and unresolved variables are initially typed as ~:dynamic~.) The resulting inferred types are used to add runtime casts at the function boundaries, preserving blame semantics of explicit annotations.
>>>>>>> t0
...
Check if a value is a registered or built-in type keyword. Returns =true= for =:number=, =:string=, =:fn= contracts, and user-defined types (via =deftype=, =defenum=, =deftrecord=). Returns =false= for non-type values.
=======
...
# typed variadic — the rest element type
...
# positional + optional + typed rest
...
<<<<<<< HEAD
An assert inside a =deftn= body that ensures the inferred type of an expression matches an expected type. Errors at macro-expansion time if the assertion fails.
=======
...
<<<<<<< HEAD
Higher-order function types =(:fn [:a :b -> :return])= wrap arguments in contracts, each call rechecks args and return types. A contract can be used to type a function. A contract can also be used as a return type (i.e. function returning function) and infer the type of values it operates on.
...
Note: arrow notation can use either '->' or ':->' as preferred, however they are only syntactic and are not a type class, nor first-class arrows.
=======
...
<<<<<<< HEAD
At some points of evaluation (e.g. arithmetic operations) and flow control (e.g. =if= branches) a variable's type can be treated as more specific (narrowed) based on use and predicate checks (=number?=, =string?=, etc.) e.g. in the true branch of =(= :keyword x)=, =x= is treated as =:keyword=.
=======
...
<<<<<<< HEAD
Unannotated parameters in =deftn= and =define= are inferred from usage in the function body, e.g. arithmetic operators (=+=, =-=, =*=, =/=) imply =:number=, the =string= function implies =:string=, etc.
=======
