...
- ~(optional name type)~
—optional positional argument (defaults to ~nil~), can also be set via ~:keyword value~
- ~(guard pred-fn)~
—optional guard predicate (for any extra validation)
- ~(print handler-fn)~
—optional custom pretty-print handler (for string based output)
