...
Define a named structural record type with typed field declarations usinf =deftrecord=. A definition will generate a constructor, accessors, mutators (if mutable), and an optional =:pp= pretty-print handler.
...
A definition can include guard predicates which have access to the declared fields. A guard predicate follows the same semantics as a type predicate. A guard can extend the validation of a single field (c.f. type narrowing) and also validate relations between fields. e.g. a guard can be used to check that an =end= field of type =:datetime= occurs later than the =start= field.
