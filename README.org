...
:PROPERTIES:
:ID: type-properties-and-inference
:END:
...
See [[#type-properties-and-inference][Type properties, checking and inference]] for the details; ~infer-expression~, ~infer-expression-full~, ~fn-type-of~, ~infer-assert-type~, ~with-inference-trace~, ~enable-inference-trace~, ~deftcheck~, and ~check-form~.
